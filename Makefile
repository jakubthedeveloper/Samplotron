...
TEST_ENV := native
KEYPAD_DIAGNOSTIC_DIR := tools/calculator_keypad_diagnostic

.PHONY: help upload-main upload-debug upload-debug-midi upload-keypad-diagnostic build-main build-debug build-debug-midi test monitor convert-samples
...
help:
	...
	@echo "  make upload-keypad-diagnostic - build and upload calculator keypad diagnostic"
...
upload-keypad-diagnostic:
	$(PIO) run --project-dir $(KEYPAD_DIAGNOSTIC_DIR) -t upload
