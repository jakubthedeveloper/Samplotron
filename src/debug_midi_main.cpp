...
uint8_t expectedDataBytes(uint8_t status) {
  ...
  switch (status & 0xF0) {
    ...
    case 0xA0:
    case 0xB0:
    // Keep message framing valid even though pitch bend events are ignored by this debug build.
    case 0xE0:
      return 2;
    case 0xC0:
    case 0xD0:
...
void printShortMessage(uint8_t status, const uint8_t *data) {
  ...
  if (type == 0x90 && data[1] > 0) return;
  if (type == 0x80 || (type == 0x90 && data[1] == 0)) return;
  if (type == 0xB0 || type == 0xC0) return;
}
